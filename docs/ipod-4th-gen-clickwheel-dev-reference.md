...
- `Amphenol SFV8R-2STE1HLF`

当前采用这个料号，不是因为“看起来差不多”，而是因为：

- `Gigahawk/clickwheel_breakout_4th_gen` 直接使用了该连接器
- 其定义与当前工程 pinout 一致：`8P / 0.5mm / top-contact / flip-lock`
- EasyEDA 系统库存在同名器件与封装，可直接落板

当前工程结论：

- `pitch` 可按 `0.5mm`
- `pin count` 可按 `8P`
- 连接器类型可按 `top-contact / flip-lock`
- 连接器额外机械固定脚不参与信号连接

仍未被文档单独证明、必须用实物复核的部分：
...
- 不要把额外机械脚当第 `9/10` 根信号脚使用
- `top-contact` 的含义是：排线露铜面在插入时必须朝向连接器触点侧
- 如果实物 wheel 的排线露铜面方向与当前板上摆放冲突，正确动作是调整连接器朝向或镜像布局，而不是在电气连接上“硬拧”

因此，当前状态不是“连接器完全没有结论”，而是：

- **料号和封装族可以先锁**
- **最终插入/翻折方向必须在 PCB 冻结前用实物再核一次**
